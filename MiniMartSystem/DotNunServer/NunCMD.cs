...
namespace MiniMartSystem.DotNunServer
{
    internal class NunCMD
    {
        ...
        public void AddNomalRecord(string table_name, string column_name_1, object control_1)
        {
            using (SqlConnection conn = new SqlConnection(Connect))
            {
                try
                {
                    ...
                    if (control_1 is TextBox textbox)
                    {
        ...
        {
            object[] textbox = new object[] { control_1, control_2, control_3, control_4, control_5, control_6 };
            using (SqlConnection conn = new SqlConnection(Connect))
            {
                try
                {
        ...
        public DataTable SelectAllNomalRecord(string table_name, string expression, string col_picture_name)
        {
            ...
            using (SqlConnection conn = new SqlConnection(Connect))
            {
                try
                {
                    ...
                    //{
                    //    if (dr[col_picture_name] == null)
                    //        continue;
                    //    else
                    //    {
                    //        dt.Columns.Add("pic", Type.GetType("System.Byte[]"));
                    //        dr["pic"] = File.ReadAllBytes(dr[col_picture_name].ToString());
